...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Shared/ParameterBindingMethodCache.cs
// Source Sha256: d796535c7fabeee2acc26d1540867fc764c3181c
...
internal sealed class ParameterBindingMethodCache<TRequestContext> where TRequestContext : class, IRequestContext
{
    private static readonly MethodInfo ConvertValueTaskMethod = typeof(ParameterBindingMethodCache<TRequestContext>).GetMethod(nameof(ConvertValueTask), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo ConvertValueTaskOfNullableResultMethod = typeof(ParameterBindingMethodCache<TRequestContext>).GetMethod(nameof(ConvertValueTaskOfNullableResult), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo BindAsyncMethod = typeof(ParameterBindingMethodCache<TRequestContext>).GetMethod(nameof(BindAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    internal static class SharedExpressions
    {
        ...
        internal static readonly ParameterExpression HttpContextExpr = Expression.Parameter(typeof(TRequestContext), "httpContext");
    }
    ...
    public Func<ParameterExpression, Expression, Expression>? FindTryParseMethod(Type type)
    {
        ...
        Func<ParameterExpression, Expression, Expression>? Finder(Type type)
        {
            ...
            {
                return (expression, formatProvider) => Expression.Call(explicitIParsableTryParseMethod, SharedExpressions.TempSourceStringExpr, formatProvider, expression);
            }
            ...
            if (type.IsEnum)
            {
                if (_enumTryParseMethod.IsGenericMethod)
                {
                    ...
                    return (expression, formatProvider) => Expression.Call(methodInfo!, SharedExpressions.TempSourceStringExpr, expression);
                }
                ...
                return (expression, formatProvider) =>
                {
                    ...
                    Expression.Assign(success, Expression.Call(_enumTryParseMethod, Expression.Constant(type), SharedExpressions.TempSourceStringExpr, enumAsObject)),
    ...
    public (Expression? Expression, int ParamCount) FindBindAsyncMethod(ParameterInfo parameter)
    {
        (Func<ParameterInfo, Expression>?, int) Finder(Type nonNullableParameterType)
        {
            ...
            if (methodInfo is null)
            {
                ...
                methodInfo = GetStaticMethodFromHierarchy(nonNullableParameterType, "BindAsync", new[] { typeof(TRequestContext), typeof(ParameterInfo) }, ValidateReturnType);

                if (methodInfo is null)
                {
                    hasParameterInfo = false;
                    methodInfo = GetStaticMethodFromHierarchy(nonNullableParameterType, "BindAsync", new[] { typeof(TRequestContext) }, ValidateReturnType);
                }
            }
                ...
                if (valueTaskResultType == nonNullableParameterType)
                {
                    return ((parameter) =>
                    {
                        ...
                        if (hasParameterInfo)
                        {
                            ...
                            typedCall = Expression.Call(methodInfo, SharedExpressions.HttpContextExpr, Expression.Constant(parameter));
                        }
                        else
                        {
                            typedCall = Expression.Call(methodInfo, SharedExpressions.HttpContextExpr);
                        }
    ...
    private static MethodInfo? GetIBindableFromHttpContextMethod(Type type)
    {
        ...
        {
            if (i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IBindableFromRequestContext<,>) && i.GetGenericArguments()[0] == type)
            {
    ...
    private static ValueTask<TValue?> BindAsync<TValue>(TRequestContext requestContext, ParameterInfo parameter)
        where TValue : class?, IBindableFromRequestContext<TValue, TRequestContext>
    {
        return TValue.BindAsync(requestContext, parameter);
    }
