...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Shared/PropertyAsParameterInfo.cs
// Source Sha256: 8799d8b76481f0bc468f5a91a2902368f3bcbe3a
...
internal sealed class PropertyAsParameterInfo<TRequestContext> : ParameterInfo where TRequestContext : class, IRequestContext
{
    ...
    public static ReadOnlySpan<ParameterInfo> Flatten(ParameterInfo[] parameters, ParameterBindingMethodCache<TRequestContext> cache)
    {
            ...
            if (parameters[i].CustomAttributes.Any(a => a.AttributeType == typeof(AsParametersAttribute)))
            {
                ...
                if (constructor is not null && constructorParameters is { Length: > 0 })
                {
                    foreach (var constructorParameter in constructorParameters)
                    {
                        flattenedParameters.Add(
                            new PropertyAsParameterInfo<TRequestContext>(
                ...
                else
                {
                    ...
                    foreach (var property in properties)
                    {
                        ...
                        {
                            flattenedParameters.Add(new PropertyAsParameterInfo<TRequestContext>(property, nullabilityContext));
                        }
                    }
                }
            }
