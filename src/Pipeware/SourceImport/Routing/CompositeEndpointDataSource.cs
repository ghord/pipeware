...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/CompositeEndpointDataSource.cs
// Source Sha256: 8c1ea4c586092bb6c7703afefdbb1a82f35c6645
...
/// <summary>
/// Represents an <see cref="EndpointDataSource{TRequestContext}"/> whose values come from a collection of <see cref="EndpointDataSource{TRequestContext}"/> instances.
/// </summary>
[DebuggerDisplay("{DebuggerDisplayString,nq}")]
public sealed class CompositeEndpointDataSource<TRequestContext> : EndpointDataSource<TRequestContext>, IDisposable where TRequestContext : class, IRequestContext
{
    private readonly object _lock = new();
    private readonly ICollection<EndpointDataSource<TRequestContext>> _dataSources;

    private List<Endpoint<TRequestContext>>? _endpoints;
    private IChangeToken? _consumerChangeToken;
    private CancellationTokenSource? _cts;
    private List<IDisposable>? _changeTokenRegistrations;
    private bool _disposed;

    internal CompositeEndpointDataSource(ObservableCollection<EndpointDataSource<TRequestContext>> dataSources)
    {
    ...
    /// <summary>
    /// Instantiates a <see cref="CompositeEndpointDataSource{TRequestContext}"/> object from <paramref name="endpointDataSources"/>.
    /// </summary>
    /// <param name="endpointDataSources">An collection of <see cref="EndpointDataSource{TRequestContext}" /> objects.</param>
    /// <returns>A <see cref="CompositeEndpointDataSource{TRequestContext}"/>.</returns>
    public CompositeEndpointDataSource(IEnumerable<EndpointDataSource<TRequestContext>> endpointDataSources)
    {
        _dataSources = new List<EndpointDataSource<TRequestContext>>();
    ...
    /// <summary>
    /// Returns the collection of <see cref="EndpointDataSource{TRequestContext}"/> instances associated with the object.
    /// </summary>
    public IEnumerable<EndpointDataSource<TRequestContext>> DataSources => _dataSources;

    /// <summary>
    /// Gets a <see cref="IChangeToken"/> used to signal invalidation of cached <see cref="Endpoint{TRequestContext}"/> instances.
    /// </summary>
    ...
    public override IReadOnlyList<Endpoint<TRequestContext>> GetGroupedEndpoints(RouteGroupContext<TRequestContext> context)
    {
        ...
        {
            return Array.Empty<Endpoint<TRequestContext>>();
        }
    ...
    public void Dispose()
    {
        ...
        lock (_lock)
        {
            ...
            if (_dataSources is ObservableCollection<EndpointDataSource<TRequestContext>> observableDataSources)
            {
    ...
    {
        var endpoints = new List<Endpoint<TRequestContext>>();
