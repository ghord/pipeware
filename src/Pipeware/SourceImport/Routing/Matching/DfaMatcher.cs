...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Matching/DfaMatcher.cs
// Source Sha256: 3bbe69e6d91c5b45da8de50085f7cf23edcad721
...
internal sealed partial class DfaMatcher<TRequestContext> : Matcher<TRequestContext> where TRequestContext : class, IRequestContext
{
    private const int CandidateSetStackSize = 4;

    private readonly ILogger _logger;
    private readonly EndpointSelector<TRequestContext> _selector;
    private readonly DfaState<TRequestContext>[] _states;
    private readonly int _maxSegmentCount;
    private readonly bool _isDefaultEndpointSelector;

    public DfaMatcher(ILogger<DfaMatcher<TRequestContext>> logger, EndpointSelector<TRequestContext> selector, DfaState<TRequestContext>[] states, int maxSegmentCount)
    {
        _logger = logger;
        _selector = selector;
        _states = states;
        _maxSegmentCount = maxSegmentCount;
        _isDefaultEndpointSelector = selector is DefaultEndpointSelector<TRequestContext>;
    }

    [SkipLocalsInit]
    public sealed override Task MatchAsync(TRequestContext requestContext)
    {
        ...
        var path = requestContext.GetRequestPathFeature().Path.Value!;
        ...
        var (candidates, policies) = FindCandidateSet(requestContext, path, segments);
        ...
        if (candidateCount == 1 && policyCount == 0 && _isDefaultEndpointSelector)
        {
            ...
            if (candidate.Flags == Candidate<TRequestContext>.CandidateFlags.None)
            {
                requestContext.SetEndpoint(candidate.Endpoint);
        ...
        CandidateState<TRequestContext>[]? candidateStateArray = null;
        ...
        var candidateState = useFastPath && candidateCount <= CandidateSetStackSize
            ? ((Span<CandidateState<TRequestContext>>)candidateStateStackArray)[..candidateCount]
            : (candidateStateArray = new CandidateState<TRequestContext>[candidateCount]);
        ...
        for (var i = 0; i < candidateCount; i++)
        {
            ...
            state = new CandidateState<TRequestContext>(candidate.Endpoint, candidate.Score);

            var flags = candidate.Flags;

            // First process all of the parameters and defaults.
            if ((flags & Candidate<TRequestContext>.CandidateFlags.HasSlots) != 0)
            {
                ...
                if ((flags & Candidate<TRequestContext>.CandidateFlags.HasDefaults) != 0)
                {
                    Array.Copy(prototype, 0, slots, 0, prototype.Length);
                }

                if ((flags & Candidate<TRequestContext>.CandidateFlags.HasCaptures) != 0)
                {
                    ProcessCaptures(slots, candidate.Captures, path, segments);
                }

                if ((flags & Candidate<TRequestContext>.CandidateFlags.HasCatchAll) != 0)
                {
            ...
            var isMatch = true;
            if ((flags & Candidate<TRequestContext>.CandidateFlags.HasComplexSegments) != 0)
            {
                ...
                {
                    CandidateSet<TRequestContext>.SetValidity(ref state, false);
                    isMatch = false;
                }
            }

            if ((flags & Candidate<TRequestContext>.CandidateFlags.HasConstraints) != 0)
            {
                state.Values ??= new RouteValueDictionary();
                if (!ProcessConstraints(candidate.Endpoint, candidate.Constraints, requestContext, state.Values))
                {
                    CandidateSet<TRequestContext>.SetValidity(ref state, false);
                    isMatch = false;
                }
        ...
        if (useFastPath)
        {
            DefaultEndpointSelector<TRequestContext>.Select(requestContext, candidateState);
            return Task.CompletedTask;
        }
        else if (policyCount == 0)
        {
            ...
            return _selector.SelectAsync(requestContext, new CandidateSet<TRequestContext>(candidateStateArray));
        }

        Debug.Assert(candidateStateArray is not null);
        return SelectEndpointWithPoliciesAsync(requestContext, policies, new CandidateSet<TRequestContext>(candidateStateArray));
    }

    internal (Candidate<TRequestContext>[] candidates, IEndpointSelectorPolicy<TRequestContext>[] policies) FindCandidateSet(
        ...
        ReadOnlySpan<PathSegment> segments)
    {
        ...
        {
            destination = policyTransitions.GetDestination(requestContext);
        ...
        RouteValueDictionary values)
    {
        for (var i = 0; i < constraints.Length; i++)
        {
            ...
            if (!constraint.Value.Match(requestContext, NullRouter.Instance, constraint.Key, values, RouteDirection.IncomingRequest))
            {
                Log.CandidateRejectedByConstraint(_logger, requestContext.GetRequestPathFeature().Path, endpoint, constraint.Key, constraint.Value, values[constraint.Key]);
                return false;
            }
        }
        ...
        CandidateSet<TRequestContext> candidateSet)
    {
        for (var i = 0; i < policies.Length; i++)
        {
            ...
            await policy.ApplyAsync(requestContext, candidateSet);
            if (requestContext.GetEndpoint() != null)
            {
        ...
        await _selector.SelectAsync(requestContext, candidateSet);
    }
...
#pragma warning disable IDE0051 // Remove unused private members
        private CandidateState<TRequestContext> _value0;
...
#pragma warning restore CA1823 // Avoid unused private fields
    ...
    private static partial class Log
    {
        ...
        public static void CandidatesFound(ILogger logger, string path, Candidate<TRequestContext>[] candidates)
        ...
        public static void CandidateRejectedByComplexSegment(ILogger logger, string path, Endpoint<TRequestContext> endpoint, RoutePatternPathSegment segment)
        {
        ...
        public static void CandidateRejectedByConstraint(ILogger logger, string path, Endpoint<TRequestContext> endpoint, string constraintName, IRouteConstraint constraint, object? value)
        {
        ...
        public static void CandidateNotValid(ILogger logger, string path, Endpoint<TRequestContext> endpoint)
        {
        ...
        public static void CandidateValid(ILogger logger, string path, Endpoint<TRequestContext> endpoint)
        {
            // This can be the fallback value because it really might not be a route endpoint
            if (logger.IsEnabled(LogLevel.Debug))
            {
        ...
        private static string GetRoutePattern(Endpoint<TRequestContext> endpoint)
        {
            return (endpoint as RouteEndpoint<TRequestContext>)?.RoutePattern?.RawText ?? "(none)";
        }
    }
}