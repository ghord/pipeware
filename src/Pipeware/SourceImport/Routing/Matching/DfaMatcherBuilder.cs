...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Matching/DfaMatcherBuilder.cs
// Source Sha256: f7e3adb6a8caf65c4542a062038f008543e91c81
...
internal sealed class DfaMatcherBuilder<TRequestContext> : MatcherBuilder<TRequestContext> where TRequestContext : class, IRequestContext
{
    private readonly List<RouteEndpoint<TRequestContext>> _endpoints = new List<RouteEndpoint<TRequestContext>>();

    private readonly ILoggerFactory _loggerFactory;
    private readonly ParameterPolicyFactory<TRequestContext> _parameterPolicyFactory;
    private readonly EndpointSelector<TRequestContext> _selector;
    private readonly IEndpointSelectorPolicy<TRequestContext>[] _endpointSelectorPolicies;
    private readonly INodeBuilderPolicy<TRequestContext>[] _nodeBuilders;
    private readonly EndpointComparer<TRequestContext> _comparer;
    ...
    public DfaMatcherBuilder(
        ...
        EndpointSelector<TRequestContext> selector,
        IEnumerable<MatcherPolicy<TRequestContext>> policies)
    {
        ...
        _comparer = new EndpointComparer<TRequestContext>(endpointComparerPolicies);
    ...
    // Used in tests
    internal EndpointComparer<TRequestContext> Comparer => _comparer;

    public override void AddEndpoint(RouteEndpoint<TRequestContext> endpoint)
    {
        _endpoints.Add(endpoint);
    }

    public DfaNode<TRequestContext> BuildDfaTree(bool includeLabel = false)
    {
        ...
        var root = new DfaNode<TRequestContext>() { PathDepth = 0, Label = includeLabel ? "/" : null };
        ...
        for (var i = 0; i < _endpoints.Count; i++)
        {
            ...
            work.Add(new DfaBuilderWorkerWorkItem(endpoint, precedenceDigit, new List<DfaNode<TRequestContext>>() { root, }));
    ...
    private sealed class CachingParameterPolicyFactory : ParameterPolicyFactory<TRequestContext>
    {
        private readonly ParameterPolicyFactory<TRequestContext> _inner;
        private readonly Dictionary<string, IParameterPolicy> _cachedParameters;

        public CachingParameterPolicyFactory(ParameterPolicyFactory<TRequestContext> inner)
        {
            _inner = inner;
    ...
    private sealed class DfaBuilderWorker
    {
        ...
        internal void ProcessLevel(int depth)
        {
            ...
            for (var i = 0; i < _workCount; i++)
            {
                ...
                List<DfaNode<TRequestContext>> nextParents;
                ...
                {
                    nextParents = new List<DfaNode<TRequestContext>>();
        ...
        private void ProcessSegment(
            ...
            List<DfaNode<TRequestContext>> parents,
            List<DfaNode<TRequestContext>> nextParents,
            RoutePatternPathSegment segment)
        {
            for (var i = 0; i < parents.Count; i++)
            {
                ...
                else if (segment.IsSimple && parameterPart != null && parameterPart.IsCatchAll)
                {
                    ...
                    {
                        parent.CatchAll = new DfaNode<TRequestContext>()
                        {
                ...
                else if (segment.IsSimple && parameterPart != null)
                {
                    if (parent.Parameters == null)
                    {
                        parent.Parameters = new DfaNode<TRequestContext>()
                        {
                            PathDepth = parent.PathDepth + 1,
                            Label = _includeLabel ? parent.Label + "{...}/" : null,
                        };
        ...
        private void AddParentsMatchingComplexSegment(RouteEndpoint<TRequestContext> endpoint, List<DfaNode<TRequestContext>> nextParents, RoutePatternPathSegment segment, DfaNode<TRequestContext> parent, RoutePatternParameterPart parameterPart)
        {
        ...
        private void AddParentsWithMatchingLiteralConstraints(List<DfaNode<TRequestContext>> nextParents, DfaNode<TRequestContext> parent, RoutePatternParameterPart parameterPart, IReadOnlyList<RoutePatternParameterPolicyReference> parameterPolicyReferences)
        {
        ...
        private void AddRequiredLiteralValue(RouteEndpoint<TRequestContext> endpoint, List<DfaNode<TRequestContext>> nextParents, DfaNode<TRequestContext> parent, RoutePatternParameterPart parameterPart, object requiredValue)
        {
            if (endpoint.RoutePattern.ParameterPolicies.TryGetValue(parameterPart.Name, out var parameterPolicyReferences))
            {
    ...
    private static void AddLiteralNode(bool includeLabel, List<DfaNode<TRequestContext>> nextParents, DfaNode<TRequestContext> parent, string literal)
    {
        if (parent.Literals == null ||
            !parent.Literals.TryGetValue(literal, out var next))
        {
            next = new DfaNode<TRequestContext>()
            {
                PathDepth = parent.PathDepth + 1,
    ...
    private static RoutePatternPathSegment GetCurrentSegment(RouteEndpoint<TRequestContext> endpoint, int depth)
    {
    ...
    private static int GetPrecedenceDigitAtDepth(RouteEndpoint<TRequestContext> endpoint, int depth)
    {
        var segment = GetCurrentSegment(endpoint, depth);
        if (segment is null)
        {
    ...
    public override Matcher<TRequestContext> Build()
    {
#if DEBUG
        ...
        var states = new DfaState<TRequestContext>[stateCount];
        var exitDestination = stateCount - 1;
        AddNode(root, states, exitDestination);

        // The root state only has a jump table.
        states[exitDestination] = new DfaState<TRequestContext>(
            Array.Empty<Candidate<TRequestContext>>(),
            Array.Empty<IEndpointSelectorPolicy<TRequestContext>>(),
            JumpTableBuilder.Build(exitDestination, exitDestination, null),
            null);

        return new DfaMatcher<TRequestContext>(_loggerFactory.CreateLogger<DfaMatcher<TRequestContext>>(), _selector, states, maxSegmentCount);
    }

    private int AddNode(
        DfaNode<TRequestContext> node,
        DfaState<TRequestContext>[] states,
        int exitDestination)
    {
        ...
        // this lazily.
        List<IEndpointSelectorPolicy<TRequestContext>> endpointSelectorPolicies = null;
        if (node.Matches?.Count > 0)
        {
            for (var i = 0; i < _endpointSelectorPolicies.Length; i++)
            {
                ...
                if (endpointSelectorPolicy.AppliesToEndpoints(node.Matches))
                {
                    ...
                    {
                        endpointSelectorPolicies = new List<IEndpointSelectorPolicy<TRequestContext>>();
                    }
        ...
        states[currentStateIndex] = new DfaState<TRequestContext>(
            candidates,
            endpointSelectorPolicies?.ToArray() ?? Array.Empty<IEndpointSelectorPolicy<TRequestContext>>(),
        ...
        int Transition(DfaNode<TRequestContext> next)
        {
            // Break cycles
    ...
    private static PolicyJumpTable<TRequestContext> BuildPolicy(int exitDestination, INodeBuilderPolicy<TRequestContext> nodeBuilder, PolicyJumpTableEdge[] policyEntries)
    {
    ...
    // endpoint.
    internal Candidate<TRequestContext>[] CreateCandidates(IReadOnlyList<Endpoint<TRequestContext>> endpoints)
    {
        if (endpoints == null || endpoints.Count == 0)
        {
            return Array.Empty<Candidate<TRequestContext>>();
        }

        var candiates = new Candidate<TRequestContext>[endpoints.Count];
    ...
    // internal for tests
    internal Candidate<TRequestContext> CreateCandidate(Endpoint<TRequestContext> endpoint, int score)
    {
        ...
        if (endpoint is RouteEndpoint<TRequestContext> routeEndpoint)
        {
            ...
            return new Candidate<TRequestContext>(
                endpoint,
                score,
                _slots.ToArray(),
                _captures.ToArray(),
    ...
    private static bool HasAdditionalRequiredSegments(RouteEndpoint<TRequestContext> endpoint, int depth)
    {
    ...
    private void ApplyPolicies(DfaNode<TRequestContext> node)
    {
        ...
        var work = new List<DfaNode<TRequestContext>>() { node, };
        List<DfaNode<TRequestContext>> previousWork = null;
        for (var i = 0; i < _nodeBuilders.Length; i++)
        {
            var nodeBuilder = _nodeBuilders[i];

            // Build a list of each
            List<DfaNode<TRequestContext>> nextWork;
            if (previousWork == null)
            {
                nextWork = new List<DfaNode<TRequestContext>>();
            }
            else
            {
            ...
            {
                var parent = work[j];
                if (!nodeBuilder.AppliesToEndpoints(parent.Matches ?? (IReadOnlyList<Endpoint<TRequestContext>>)Array.Empty<Endpoint<TRequestContext>>()))
                {
                ...
                var edges = nodeBuilder.GetEdges(parent.Matches ?? (IReadOnlyList<Endpoint<TRequestContext>>)Array.Empty<Endpoint<TRequestContext>>());
                for (var k = 0; k < edges.Count; k++)
                {
                    var edge = edges[k];

                    var next = new DfaNode<TRequestContext>()
                    {
    ...
    private static (INodeBuilderPolicy<TRequestContext>[] nodeBuilderPolicies, IEndpointComparerPolicy<TRequestContext>[] endpointComparerPolicies, IEndpointSelectorPolicy<TRequestContext>[] endpointSelectorPolicies) ExtractPolicies(IEnumerable<MatcherPolicy<TRequestContext>> policies)
    {
        var nodeBuilderPolicies = new List<INodeBuilderPolicy<TRequestContext>>();
        var endpointComparerPolicies = new List<IEndpointComparerPolicy<TRequestContext>>();
        var endpointSelectorPolicies = new List<IEndpointSelectorPolicy<TRequestContext>>();

        foreach (var policy in policies)
        {
            if (policy is INodeBuilderPolicy<TRequestContext> nodeBuilderPolicy)
            {
                nodeBuilderPolicies.Add(nodeBuilderPolicy);
            }

            if (policy is IEndpointComparerPolicy<TRequestContext> endpointComparerPolicy)
            {
                endpointComparerPolicies.Add(endpointComparerPolicy);
            }

            if (policy is IEndpointSelectorPolicy<TRequestContext> endpointSelectorPolicy)
            {
                endpointSelectorPolicies.Add(endpointSelectorPolicy);
            }
        }
    ...
    {
        public RouteEndpoint<TRequestContext> Endpoint { get; }

        public int PrecedenceDigit { get; }

        public List<DfaNode<TRequestContext>> Parents { get; }

        public DfaBuilderWorkerWorkItem(RouteEndpoint<TRequestContext> endpoint, int precedenceDigit, List<DfaNode<TRequestContext>> parents)
        {
            Endpoint = endpoint;
        ...
        public void Deconstruct(out RouteEndpoint<TRequestContext> endpoint, out int precedenceDigit, out List<DfaNode<TRequestContext>> parents)
        {
            endpoint = Endpoint;
            precedenceDigit = PrecedenceDigit;
            parents = Parents;
