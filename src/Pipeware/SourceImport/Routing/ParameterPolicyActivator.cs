...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/ParameterPolicyActivator.cs
// Source Sha256: 3410dc09bf4f59bcd35d022c07cba3abb14e7122
...
string.Format("The constraint type '{0}' which is mapped to constraint key '{1}' must implement the '{2}' interface.", parameterPolicyType, parameterPolicyKey, typeof(T).Name));
            }
    ...
    private static IParameterPolicy CreateParameterPolicy(IServiceProvider? serviceProvider, Type parameterPolicyType, string? argumentString)
    {
        ...
        else
        {
            ...
            {
                throw new RouteCreationException(
string.Format("Could not find a constructor for constraint type '{0}' with the following number of parameters: {1}.", parameterPolicyType.Name, arguments.Length));
            }
            else
            {
    ...
    private static ConstructorInfo GetLongestConstructor(List<ConstructorInfo> constructors, Type parameterPolicyType)
    {
        ...
        {
            throw new RouteCreationException(
string.Format("The constructor to use for activating the constraint type '{0}' is ambiguous. Multiple constructors were found with the following number of parameters: {1}.", parameterPolicyType.Name, longestLength));
        }
