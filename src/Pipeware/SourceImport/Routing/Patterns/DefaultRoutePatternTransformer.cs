...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Patterns/DefaultRoutePatternTransformer.cs
// Source Sha256: ae65ee50e8e484e79679adaf61f274c6092f0ea0
...
internal sealed class DefaultRoutePatternTransformer<TRequestContext> : RoutePatternTransformer<TRequestContext> where TRequestContext : class, IRequestContext
{
    private readonly ParameterPolicyFactory<TRequestContext> _policyFactory;

    public DefaultRoutePatternTransformer(ParameterPolicyFactory<TRequestContext> policyFactory)
    {
    ...
    private bool MatchesConstraints(RoutePattern pattern, RoutePatternParameterPart parameter, string key, RouteValueDictionary requiredValues)
    {
        if (pattern.ParameterPolicies.TryGetValue(key, out var policies))
        {
            for (var i = 0; i < policies.Count; i++)
            {
                ...
                {
                    if (!constraint.Match(requestContext: null, NullRouter.Instance, key, requiredValues, RouteDirection.IncomingRequest))
                    {
                        return false;
                    }
                }
