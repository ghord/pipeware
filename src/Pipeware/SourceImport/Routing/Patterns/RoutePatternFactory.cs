...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Patterns/RoutePatternFactory.cs
// Source Sha256: ff1336717798b1cf491da5f6288863d637fd44e1
...
#endif
{
    ...
    private static Dictionary<string, List<RoutePatternParameterPolicyReference>>? CreateRoutePatternPolicyReferences(RouteValueDictionary? parameterPolicies)
    {
        ...
        if (parameterPolicies != null && parameterPolicies.Count > 0)
        {
            ...
            foreach (var kvp in parameterPolicies)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format("Invalid constraint '{0}'. A constraint must be of type 'string' or '{1}'.", kvp.Value ?? "null", typeof(IRouteConstraint)));
                }
        ...
        RoutePatternParameterKind parameterKind)
    {
        ...
        {
            throw new ArgumentException("An optional parameter cannot have default value.");
        }
    ...
    public static RoutePatternParameterPolicyReference Constraint(object constraint)
    {
        ...
        {
            throw new InvalidOperationException(string.Format("Invalid constraint '{0}'. A constraint must be of type 'string' or '{1}'.", constraint ?? "null", typeof(IRouteConstraint)));
        }
    }
    ...
    /// </summary>
    /// <param name="constraint">
    /// The constraint text, which will be resolved by <see cref="ParameterPolicyFactory{TRequestContext}"/>.
    /// </param>
    /// <returns>The <see cref="RoutePatternParameterPolicyReference"/>.</returns>
#endif
    ...
    /// </summary>
    /// <param name="parameterPolicy">
    /// The parameter policy text, which will be resolved by <see cref="ParameterPolicyFactory{TRequestContext}"/>.
    /// </param>
    /// <returns>The <see cref="RoutePatternParameterPolicyReference"/>.</returns>
#endif
    ...
    public static RoutePattern Combine(RoutePattern? left, RoutePattern right)
    {
            ...
            string dictionaryName)
        {
            ...
            foreach (var (key, value) in rightDictionary)
            {
                if (!combinedDictionary.TryAdd(key, value) && !Equals(combinedDictionary[key], value))
                {
                    ...
                    throw new InvalidOperationException(string.Format("MapGroup cannot build a pattern for '{0}' because the 'RoutePattern.{1}' dictionary key '{2}' has multiple values.", rawText, dictionaryName, key));
                }
            }
        ...
        static Action<RoutePatternParameterPart> CheckDuplicateParameters(int parameterCount, string rawText)
        {
            ...
            return parameterPart =>
            {
                ...
                {
                    var errorText = string.Format("The route parameter name '{0}' appears more than one time in the route template.", parameterPart.Name);
