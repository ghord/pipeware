...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Patterns/RoutePatternParser.cs
// Source Sha256: 31ae156ea5300dde030a324e609633cae855cfa0
            ...
            if (part is RoutePatternParameterPart parameter && parameter.IsOptional && parts.Count > 1)
            {
                ...
                if (i == parts.Count - 1)
                {
                    ...
                    if (!previousPart.IsLiteral && !previousPart.IsSeparator)
                    {
                        ...
                        context.Error = string.Format("In the segment '{0}', the optional parameter '{1}' is preceded by an invalid segment '{2}'. Only a period (.) can precede an optional parameter.", RoutePatternPathSegment.DebuggerToString(parts), parameter.Name, parts[i - 1].DebuggerToString());
                ...
                else
                {
                    ...
                    context.Error = string.Format("An optional parameter must be at the end of the segment. In the segment '{0}', optional parameter '{1}' is followed by '{2}'.", RoutePatternPathSegment.DebuggerToString(parts), parameter.Name, parts[i + 1].DebuggerToString());
