...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Http.Abstractions/src/Routing/RouteValueDictionary.cs
// Source Sha256: 9baed10bc1226816486c968424c1d5f3c893b6d6
...
#endif
{
    ...
    public void Add(string key, object? value)
    {
        ...
        {
#if !COMPONENTS
            var message = string.Format("An element with the key '{0}' already exists in the {1}.", key, nameof(RouteValueDictionary));
    ...
    internal sealed class PropertyStorage
    {
        ...
        private static void ValidatePropertyNames(Type type, PropertyHelper[] properties)
        {
            ...
            for (var i = 0; i < properties.Length; i++)
            {
                ...
                {
                    var message = string.Format("The type '{0}' defines properties '{1}' and '{2}' which differ only by casing. This is not supported by {3} which uses case-insensitive comparisons.", type.FullName, property.Name, duplicate.Name, nameof(RouteValueDictionary));
