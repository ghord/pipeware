...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/RouteValuesAddressScheme.cs
// Source Sha256: 2a2020a9ed5353eb253ff3465c24402f372b085e
...
internal sealed class RouteValuesAddressScheme<TRequestContext> : IEndpointAddressScheme<RouteValuesAddress, TRequestContext>, IDisposable where TRequestContext : class, IRequestContext
{
    private readonly DataSourceDependentCache<StateEntry, TRequestContext> _cache;

    public RouteValuesAddressScheme(EndpointDataSource<TRequestContext> dataSource)
    {
        _cache = new DataSourceDependentCache<StateEntry, TRequestContext>(dataSource, Initialize);
    }

    // Internal for tests
    internal StateEntry State => _cache.EnsureInitialized();

    public IEnumerable<Endpoint<TRequestContext>> FindEndpoints(RouteValuesAddress address)
    {
        ...
        if (matchResults != null)
        {
            ...
            if (matchCount > 0)
            {
                if (matchResults.Count == 1)
                {
                    ...
                    return new[] { (RouteEndpoint<TRequestContext>)matchResults[0].Match.Entry.Data };
                }
                else
                {
    ...
    private static IEnumerable<Endpoint<TRequestContext>> GetEndpoints(IList<OutboundMatchResult> matchResults, int matchCount)
    {
        for (var i = 0; i < matchCount; i++)
        {
            yield return (RouteEndpoint<TRequestContext>)matchResults[i].Match.Entry.Data;
        }
    }

    private StateEntry Initialize(IReadOnlyList<Endpoint<TRequestContext>> endpoints)
    {
