...
// Source file: https://github.com/dotnet/aspnetcore/tree/release/8.0/src/Http/Routing/src/Tree/TreeRouter.cs
// Source Sha256: d3e4e9aa3362d58e62bcc39b54d6081fa2af891d
...
string.Format("Two or more routes named '{0}' have different templates.", entry.RouteName),
...
#endif
    {
        ...
        {
#if !COMPONENTS
            var tokenizer = new PathTokenizer(context.RequestContext.GetRequestPathFeature().Path);
            var root = tree.Root;
            ...
            while (treeEnumerator.MoveNext())
            {
                ...
                foreach (var item in node.Matches)
                {
                    ...
                    try
                    {
                        if (!matcher.TryMatch(context.RequestContext.GetRequestPathFeature().Path, context.RouteData.Values))
                        {
                            continue;
                        }
